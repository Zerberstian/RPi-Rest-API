# Flask related Imports
from flask import Flask, request, jsonify
from flask_sqlalchemy import SQLAlchemy
from flask_restful import Resource, Api, reqparse, fields, marshal_with, abort
# Other Imports
from pathlib import Path
import os

file_path = Path(__file__).parent / "x-api_key.txt"

def read_text_file(path):
    with open(path, "r", encoding="utf-8") as f:
        return f.read().strip()

expected_api_key = read_text_file(file_path)
#print(f"\nKey: {expected_api_key}\n")


app = Flask(__name__)
basedir = os.path.abspath(os.path.dirname(__file__))
app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{os.path.join(basedir, "instance", "database.db").replace(chr(92), "/")}'
db = SQLAlchemy(app)
api = Api(app)

class PlantModel(db.Model):
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    plant_name = db.Column(db.String(100), nullable=False)
    waterlevel = db.Column(db.Integer, nullable=False)

    def __repr__(self):
        return f'Plant(id={self.id}, plant_name={self.plant_name}, waterlevel={self.waterlevel})'


plant_args = reqparse.RequestParser()
plant_args.add_argument('plant_name', type=str, help='Plant name is required', required=False, location='json')
plant_args.add_argument('waterlevel', type=int, help='Water level is required', required=True, location='json')

plantFields = {
    'id': fields.Integer,
    'plant_name': fields.String,
    'waterlevel': fields.Integer
} 

def check_api_key():
    received_api_key = request.headers.get("X-API-Key", "").strip()

    if received_api_key != expected_api_key:
        abort(401, message="Unauthorized: Invalid API Key")


# ============================================================
# /api/plants/
# ============================================================
class Plants(Resource):

    # GET ALL PLANTS
    @marshal_with(plantFields)
    def get(self):
        check_api_key()

        plants = PlantModel.query.all()

        return plants, 200

    # CREATE PLANT
    @marshal_with(plantFields)
    def post(self):
        check_api_key()

        args = plant_args.parse_args()

        plant = PlantModel(
            plant_name=args["plant_name"],
            waterlevel=args["waterlevel"]
        )

        db.session.add(plant)
        db.session.commit()

        return plant, 201


# ============================================================
# /api/plants/<id>
# ============================================================

class Plant(Resource):

    # GET ONE PLANT
    @marshal_with(plantFields)
    def get(self, id):
        check_api_key()

        plant = PlantModel.query.filter_by(id=id).first()

        if not plant:
            abort(404, message="Plant not found")

        return plant, 200

    # UPDATE PLANT
    @marshal_with(plantFields)
    def patch(self, id):
        check_api_key()

        plant = PlantModel.query.filter_by(id=id).first()

        if not plant:
            abort(404, message="Plant not found")

        args = plant_args.parse_args()

        # Only update fields that were actually supplied
        if args["plant_name"] is not None:
            plant.plant_name = args["plant_name"]

        if args["waterlevel"] is not None:
            plant.waterlevel = args["waterlevel"]

        db.session.commit()

        return plant, 200

    # DELETE PLANT
    def delete(self, id):
        check_api_key()

        plant = PlantModel.query.filter_by(id=id).first()

        if not plant:
            abort(404, message="Plant not found")

        db.session.delete(plant)
        db.session.commit()

        return {
            "message": "Plant deleted successfully"
        }, 200

    '''   
    @marshal_with(plantFields)
    def delete(self, id):
        # Key validation
        received_api_key = request.headers.get("X-API-Key", "")
        if received_api_key != expected_api_key:
            abort(401, message="Unauthorized: Invalid API Key")

        plant = PlantModel.query.filter_by(id=id).first()

        if not plant: 
            abort(404, message="Plant not found lol")

        db.session.delete(plant)
        db.session.commit()

        plants = PlantModel.query.all()
        return plants, 204
'''

api.add_resource(Plants, '/api/plants/')
api.add_resource(Plant, '/api/plants/<int:id>')

@app.route('/')
def home():
    return '<h1>Flask REST APi</h1>'

print("======== ROUTES ========")
print(app.url_map)
print("========================")

if __name__ == '__main__':
    app.run(debug=True, host="192.168.189.22", port=5000)
